...
public class DecoBlockFlowerPot extends FCBlockFlowerPot
{
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		...
		if (currentItem == null && blockMetadata != 0)
		{
			world.playSoundEffect((double)x, (double)y, (double)z, "dig.grass1", 1.0F, world.rand.nextFloat() * 0.1F + 0.9F);

			player.inventory.setInventorySlotContents(player.inventory.currentItem, (ItemStack)this.getPlantForMeta(blockMetadata));
			world.setBlockMetadataWithNotify(x, y, z, 0);

			return true;
		}
		else if (blockMetadata == this.GetMetadataForItemStack(currentItem) && blockMetadata != 0)
		{
			world.playSoundEffect((double)x, (double)y, (double)z, "dig.grass1", 1.0F, world.rand.nextFloat() * 0.1F + 0.9F);

			player.inventory.addItemStackToInventory((ItemStack)this.getPlantForMeta(blockMetadata));
			world.setBlockMetadataWithNotify(x, y, z, 0);

			return true;
		}
		else if (currentItem != null && blockMetadata == 0)
		{
			...
			if (var11 > 0)
			{
				...
				if (!player.capabilities.isCreativeMode)
				{
					...
					if (currentItem.stackSize <= 0) player.inventory.setInventorySlotContents(player.inventory.currentItem, (ItemStack)null);
				}
	...
	public static int getMetaForPlant(ItemStack itemStack)
	{
		...
		else if (itemID == DecoModuleDecoration.decoItemBamboo.itemID) return 12;
		else
		{
	...
	public static ItemStack getPlantForMeta(int metadata)
	{
		switch (metadata)
		{
			...
			case 12 : return new ItemStack(DecoModuleDecoration.decoItemBamboo);
